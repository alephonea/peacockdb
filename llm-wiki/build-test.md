...
| Batch-partitioned corpus, device (Rust) | the same `corpus_query!` lines read from the other side: each enabled (query, mode) runs on a device and asserts, read-only, against the section the cpu authored — plan shape, `in_rows`, the per-batch lists and the bytes — plus the result where `gpu_oracle` names a golden. Seventeen cells: `q6`, `nested-loop-join` and `shuffle-stddev` at every mode, `q19` and `tpcds/q84` at `bp-tp1-single`. The device-side causes keeping the rest off are [#152](tickets.md#t152), [#184](tasks/bp-tickets.md#t184), [#185](tasks/bp-tickets.md#t185), [#187](tasks/bp-tickets.md#t187), [#191](tasks/bp-tickets.md#t191) and [#195](tasks/bp-tickets.md#t195); the others are queries that run on neither engine | [test_gpu_bp_corpus](../peacockdb-core/tests/test_gpu_bp_corpus.rs) | shad-gpu | 18 |
