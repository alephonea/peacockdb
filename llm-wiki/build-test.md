...
| Batch-partitioned end to end (Rust) | SQL in, rows out: 17 queries planned and run at all five modes against DataFusion on the same SQL, plus `in_flight_bytes` back to zero and holds equal releases at the end of every run — and four cases no query list can carry: that DataFusion's partial aggregate does not skip grouping here, the call and pull counts a limit makes, the smallest budget a query fits in completing where the byte below it trips, and the model compared against what the calls measured. The first tier where the planner, the recipes, the executors and both drivers run together rather than each against a fixture of the last one's shape — so what it tests is the joins between them, which four tasks of separate proofs cannot reach | [test_cpu_batch_partitioned](../peacockdb-core/tests/test_cpu_batch_partitioned.rs) | cpp-cpu | 21 |
...
the 1567. Everything else in the repo that can be enumerated as a test case is counted.
