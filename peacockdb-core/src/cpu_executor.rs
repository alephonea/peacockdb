...
//!
//! This is the single source of truth for node `output_bytes` on BOTH backends —
//! the GPU backend reconstructs its stats through `logical_size_from_schema` — so
//! CPU and GPU costs are identical by construction whenever row counts match.
//!
//! The executors themselves moved to [`crate::executors`]; this module keeps the
//! accounting helpers and re-exports the stats types so existing
//! `peacockdb_core::cpu_executor::…` paths keep resolving.
//! TODO(Inc3): these helpers move to a memory.rs/cost.rs support file.
