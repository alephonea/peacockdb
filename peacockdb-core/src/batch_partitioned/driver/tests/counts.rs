...
//!
//! The identity they rest on is that a node's `consumed` for one child totals that child's
//! `emitted`, per the child's own lane. It is an equality on every node of every run that
//! drained, and the one thing that weakens it is an early exit, which leaves whatever was
//! queued at that moment unconsumed — at whichever node the stop caught it, which is not
//! the node carrying the limit. An inequality everywhere would pass for almost anything, so
//! the cases below pin which nodes are at equality as much as which are not.
...
/// `consumed` against the child it names, lane for lane. The check the goldens make, made
/// here on the report they make it from.
fn assert_consumed_matches_children(report: &RunReport, children_of: &[Vec<usize>]) {
    for (node, children) in children_of.iter().enumerate() {
        for (slot, child) in children.iter().enumerate() {
            for lane in 0..report.lanes_of[*child] {
                assert_eq!(
                    report.consumed[node][slot][lane],
                    rows_of(report, *child, lane).iter().sum::<u64>(),
                    "node {node} slot {slot} lane {lane}: consumed against child {child}"
                );
            }
        }
    }
...
fn every_batch_a_node_emitted_is_in_the_report_in_order() {
    ...
    let report = run(unload(filter(source("part", 1))).as_ref(), &script);
    assert_eq!(
        rows_of(&report, 2, 0),
        vec![10, 7],
...
fn a_map_node_consumed_exactly_what_its_child_emitted_lane_for_lane() {
    ...
    let report = run(unload(project(filter(source("part", 3)))).as_ref(), &script);
    // 0 unload, 1 project, 2 filter, 3 source.
    assert_consumed_matches_children(&report, &[vec![1], vec![2], vec![3], vec![]]);
...
fn an_emitter_redistributes_and_its_consumed_still_names_the_lane_it_read() {
    ...
    let report = run(unload(merge(emit(source("part", 1), 4))).as_ref(), &script);
...
fn a_partition_accumulator_names_the_lane_each_batch_came_from() {
    ...
    let report = run(unload(merge(source("part", 4))).as_ref(), &script);
    // 0 unload, 1 merge, 2 source.
    assert_eq!(report.consumed[1][0], vec![10, 3, 0, 8]);
    assert_consumed_matches_children(&report, &[vec![1], vec![2], vec![]]);
    assert_accounted(&report);
}
...
fn a_node_whose_limit_was_satisfied_consumed_every_batch_that_reached_it() {
    ...
    let report = run(
        unload_limited(filter(source("part", 1)), 0, Some(15)).as_ref(),
        &script,
    );
...
/// wherever the stop caught one, which here is a lane the merge never got to and is nowhere
/// near the node carrying the limit.
#[test]
fn an_early_exit_leaves_a_batch_unconsumed_below_the_node_that_stopped() {
    ...
    let report = run(
        unload_limited(merge(source("part", 3)), 0, Some(5)).as_ref(),
        &script,
    );
    // 0 unload, 1 merge, 2 source.
    assert!(report.early_exit);
...
fn a_join_with_no_build_side_still_consumes_the_probe_batches_it_drops() {
    ...
    assert_consumed_matches_children(
        &report,
        &[vec![1], vec![2, 4], vec![3], vec![], vec![5], vec![]],
    );
    assert_accounted(&report);
}
...
fn a_released_skip_prefix_counts_as_consumed_and_emits_nothing() {
    let script = Script::default().source("part", vec![vec![spec(10, 80); 6]]);
    let report = run(
        unload_limited(filter(source("part", 1)), 25, Some(10)).as_ref(),
        &script,
    );
    assert_eq!(
